...
def embed_func_pd(
    ...
    *,
    ...
    hashvalues = np.vstack([permuted_hashvalues, masks]).min(axis=0)
    Hs = [bytes(hashvalues[start:end].byteswap().data) for start, end in hashranges]
    return pandas.Series({"__signatures__": Hs, "__id__": idx})
...
if __name__ == "__main__":  # pragma: no cover
    ...
    with timer("Total"):
        ...
        ds_pd = pd.DataFrame(ds.to_pandas(), chunk_size=1000).execute()
        DATA_SIZE = len(ds)
        PERMUTATIONS = np.array(
            [
                (
        ...
        with timer("MinHashing"):
            embedded_pd = ds_pd.apply(
                lambda row: embed_func_pd(
                    row["text"],
                    row["id"],
        ...
        with timer("Clustering"):
            embedded_pd['__signatures__'] = embedded_pd['__signatures__'].apply(lambda x: list(enumerate(x)))
            embedded_pd = embedded_pd.explode('__signatures__')
            clusters = embedded_pd.groupby('__signatures__')['__id__'].apply(set)
            import pdb; pdb.set_trace()
            # min_elems = clusters.apply(min)
        ...
        with timer("Filtering"):
            final_data = ds_pd[ds_pd['id'].map(uff.find) == ds_pd['id']]

            # ds_pd['__cluster__'] = ds_pd['id'].map(uff.find)
            # final_data = ds_pd.drop_duplicates(subset='__cluster__').drop(['__cluster__'], axis=1)
            # import pdb; pdb.set_trace()

        with timer("Saving"):
            a = 1
            # final_data.to_feather(os.path.join(args.output, "dedup.feather"))
            # if args.debug:
